...
print_flag = 0
logfile_name = 'log_modelpredictions_questions+image+captions'
...
class baseline_model(nn.Module):
   ...
   def __init__(self, question_embed_dim, caption_embed_dim, answer_embed_dim, num_classes, num_answerclasses):
      ...
      self.lstm = nn.LSTM(question_embed_dim*6, hidden_dim, 2, batch_first = True, bidirectional=True)
...
def gen():
  ...
  for i, data in enumerate(val_loader):
    ...
    hk.write("  The predicted during test was " +  predicted_answer + str(question_id.data[0]) + '\n')
    hk.write('\n')
    hk.close()
    print("I am here")
    return 
