...
class CaptionRNN_VQVED(nn.Module):
    def __init__(self, feature_size, embed_size_image, embed_size, hidden_size, vocab_size, num_layers, max_seq_length=20):
        """Set the hyper-parameters and build the layers."""
        super(CaptionRNN_VQVED, self).__init__()
        self.embed_size = embed_size
        self.embed = nn.Embedding(vocab_size, embed_size)
        self.lstm = nn.LSTM(128+embed_size, hidden_size, num_layers, batch_first=True)
        self.lstm = nn.LSTM(embed_size, hidden_size, num_layers, batch_first=True)
        self.encoder_linear = layers.SequenceWise(nn.Linear(2048, hidden_size))
        self.max_seg_length = max_seq_length
        ...
        ### Vector Quantization
        self.dimensions_vq = hidden_size
    ...
    def forward(self, features, captions):
        ...
        # Encoder
        if print_flag:
           print("  Model: Shape of features: ", features.shape)
        features = features.unsqueeze(1)
        assert len(features.shape) == 3
        encoded = torch.tanh(self.encoder_linear(features))
        if print_flag:
           print("  Model: Shape of encoder output: ", encoded.shape)

        ### Quantization
        latents = self.quantizer(encoded)
        if print_flag:
           print("  Model: Shape of output from the quantizer: ", latents.shape)
           print("  Model: Shape of captions: ", captions.shape )


        ### Decoder
        captions = self. embed(captions)
        if print_flag:
           print("  Model: Shape of embedded captions: ", captions.shape)
        
        inputs = torch.cat((latents, captions), 1)
        if print_flag:
           print("  Model: Shape of cat inputs: ", inputs.shape)
        outputs, _ = self.lstm(inputs)        
        outputs = self.final_linear(outputs)
        if print_flag:
           print("Shape of outputs from the model: ", outputs.shape)
    ...
    def sample(self, features, max_len=20):
        ...
        ### Decoder
        captions = latents.new(latents.shape[0],1)
        captions.zero_()
        captions = self.embed(captions.long())
        if print_flag:
           print("  Model: Shape of embedded captions: ", captions.shape)
        ...
        outputs = []
        states = None 
        _, states = self.lstm(latents, None)
