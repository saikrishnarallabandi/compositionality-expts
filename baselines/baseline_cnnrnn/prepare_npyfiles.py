...
def train(train_loader, encoder, decoder, criterion, optimizer, vocab_size,
    ...
    for i_step, data in enumerate(train_loader):
        ...
        if i_step % 100 == 2:
           g = open('preprocess.log','a')
           g.write("Processed " + str(i_step) + " batches" + '\n')
           g.close()
           np.save('features_val.npy',features_array)
           with open('captions_val.pkl','wb') as f: # Numpy arrays cannot store variable length batches
               pickle.dump(captions_array, f) 
           #sys.exit()
        ...
        features = encoder.get_resnet_feats(images)
        features_array.append(features.detach().cpu().numpy())
        captions_array.append(captions.detach().cpu().numpy())
