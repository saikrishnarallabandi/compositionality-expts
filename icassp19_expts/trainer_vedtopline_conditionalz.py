...
def val():
    ...
    with torch.no_grad():
       for i, (features, captions, lengths, image_names) in enumerate(val_loader):
            ...
            outputs = model.sample_greedy_ConditionalZ(features)
...
def train():
    ...
    for i, (features, captions, lengths, image_names) in enumerate(train_loader):
            ...
            outputs, mu, logvar = model.forward_ConditionalZ(features, captions)
            # captions = captions[:,1:]
