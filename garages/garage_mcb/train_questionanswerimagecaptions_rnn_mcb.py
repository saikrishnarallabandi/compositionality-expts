...
print_flag = 0
logfile_name = 'mcb_testing_rnn.txt'
...
class baseline_model(nn.Module):
   ...
   def __init__(self, question_embed_dim, caption_embed_dim, answer_embed_dim, num_classes, num_answerclasses):
      ...
      self.input_size2 = 1024
      self.output_size = 8000

      self.mcb = CompactBilinearPooling(self.input_size1, self.input_size2, self.output_size).cuda()
      self.lstm_question_ctx = nn.LSTM(question_embed_dim*2, hidden_dim*2, 2, batch_first = True, bidirectional=True)
      self.mcb_fc_1 = nn.Linear(8000, 12000)
      self.mcb_dropout_1 = nn.Dropout(0.4)
      self.act_1 = nn.Tanh()
      self.mcb_fc_2 = nn.Linear(12000, 4000)
      self.mcb_dropout_2 = nn.Dropout(0.4)
      self.act_2 = nn.Tanh()
      self.mcb_fc_3 = nn.Linear(4000, num_answerclasses)
      
      self.lstm = nn.LSTM(2048, hidden_dim, 2, batch_first = True, bidirectional=True)
   ...
   def forward(self,question, features, captions):
      ...
      x, hidden = self.lstm_question_ctx(question_embedding)
      question_ctx = hidden[0][-1]

      features = F.relu(self.image_embedding(features))

      if print_flag:
         print("Shape of question embedding: ", question_ctx.size())
      ...
      if print_flag:
         print("Shape of question embedding: ", question_ctx.size())
         print("Shape of features: ", features.size())
         print("Shape of mcb: ", mcb_embedding.size())

      total_embedding = self.mcb_fc_1(mcb_embedding)
      if self.train:
          total_embedding = self.mcb_dropout_1(total_embedding)
      total_embedding = self.mcb_fc_2(self.act_1(total_embedding))
      if self.train:
          total_embedding = self.mcb_dropout_2(total_embedding)
      total_embedding = self.mcb_fc_3(self.act_2(total_embedding))
