...
class vqa_dataset(Dataset):
   ...
   # Dataset for utterances and types
    def __init__(self, file, train_flag, wids=None):
        ...
        wids['<eos>'] = 2
        wids['UNK'] = 3
        word_tokens = self.remove_rarewords(file, 0)
        #sys.exit()
        f = open(file)
        c = 0
        for line in f:
    ...
    def remove_rarewords(self, file, freq):
       ...
       for line in f:
           ...
           #    continue
           line =  line.split('\n')[0]
           line = line.split() + ['_PAD'] + ['<sos>'] + ['<eos>'] + ["UNK"] # Punctuation and stuff 
...
def _pad(seq, max_len):
    max_len = 20
    seq = seq if len(seq) < max_len else seq[0:max_len]
