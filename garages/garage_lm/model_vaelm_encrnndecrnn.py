...
class VAEModel(nn.Module):
    ...
    def __init__(self,rnn_type,ntoken, ninp, nhid, nlayers, dropout=0.5, tie_weights=False):
       ...
       self.rnn = nn.LSTM(ninp, int(nhid/2), num_layers=1, bidirectional=True, batch_first=True)
       self.dec_rnn = nn.LSTM(int(self.nlatent/2)+int(self.ntype_emb/2), int(ntoken/4), num_layers=1, bidirectional=True, batch_first=True)

       # conditioning layers
       #self.fc5_a = nn.Linear(self.nlatent,int(self.nlatent/2))
       self.fc4 = SequenceWise(nn.Linear(int(ntoken/4)*2, ntoken))  # combines the two
    ...
    def encoder_sample(self, emb, hidden):
       logging.debug("In Encoder")
       output, hidden = self.rnn(emb, hidden)
       ...
       return self.fc2_a(h1), self.fc2_b(h1), hidden

    def forward(self,x,hidden_enc, hidden_dec ,condition):
       ...
       mu, log_var = self.encoder(embedding,hidden_enc)
    ...
    def forward_sample(self,x,enc_hidden, dec_hidden, condition): #new_input_token, enc_hidden, dec_hidden, data_type
       logging.debug("Shape of input to VAELM is {}".format(x.shape)) # [35, 20]
       embedding = self.embedding(x) # [35, 20, 200]
       logging.debug("Shape of embedding: {}".format(embedding.shape))
       mu, log_var, enc_hidden = self.encoder_sample(embedding, enc_hidden)
       logging.debug("Shape of mu after encoder: {}".format(mu.shape))
       ...
       decoded, dec_hidden = self.decode_sample(z, condition, dec_hidden)
       logging.debug("Shape of decoder output: {}".format(decoded.shape))
       return decoded,mu,log_var, enc_hidden, dec_hidden
    ...
    def decode(self,z, c):
       ...
       output, hidden = self.dec_rnn(h5, None)
       if print_flag:
    ...
    def decode_sample(self,z, c,dec_hidden):
       h3 = F.relu(self.fc3(z)) # [256, 16, 64] -> batch X seq X emb
       h4 = F.relu(self.fc5(c)) #  [256, 1, 15]
       ...
       output, hidden = self.dec_rnn(h5, dec_hidden)
       return self.fc4(output), hidden
