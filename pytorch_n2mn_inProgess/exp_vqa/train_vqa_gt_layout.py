...
from models_vqa.create_layout_pytorch import * #Seq2SeqModel

pytorch_model = Seq2SeqModel()
...
print("num_vocab_txt: ", num_vocab_txt, " embed_dim_txt= ", embed_dim_txt, " num_vocab_nmn= ", num_vocab_nmn,
      "decoder_sampling= ", decoder_sampling, " num_choices= ", num_choices)
...
loss_cret = SequenceCrossEntropy()
optim_model = torch.optim.Adam(pytorch_model.parameters(), lr=0.001, weight_decay=0.00001)
...
def run_training(max_iter, dataset_trn):
    ...
    for n_iter, batch in enumerate(dataset_trn.batches()):
        input_seq_batch = torch.LongTensor(batch['input_seq_batch'])
        seq_length_batch = torch.tensor(batch['seq_length_batch'])
        gt_layout_batch = torch.LongTensor(batch['gt_layout_batch'])
        gt_layout_batch_lens = torch.tensor(batch['gt_layout_batch_lens'])

        if torch.cuda.is_available():
            input_seq_batch = input_seq_batch.cuda()
            gt_layout_batch = gt_layout_batch.cuda()

        logits, generated, char_lengths = pytorch_model(input_seq_batch, seq_length_batch, gt_layout_batch, gt_layout_batch_lens) 
        loss = loss_cret((logits, generated, char_lengths), gt_layout_batch)
        print(loss)
        loss.backward() 
        optim_model.step()
        optim_model.zero_grad()
