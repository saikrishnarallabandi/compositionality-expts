...
class custom_loss(nn.Module):
    ...
    def forward(self, neg_entropy, answer_loss, policy_gradient_losses=None,layout_loss =None, caption_loss=None):
        ...
        if layout_loss is None:
            return torch.mean(neg_entropy) * self.lambda_entropy +\
               torch.mean(answer_loss)+torch.mean(policy_gradient_losses) + caption_loss, answer
        else:
            return answer + layout_loss + caption_loss, answer
