...
from models.caption_generation import *
...
class end2endModuleNet(nn.Module):
    def __init__(self, num_vocab_txt, num_vocab_nmn, out_num_choices,
                ...
                hidden_size, assembler, layout_criterion, answer_criterion,max_layout_len, num_c_vocab_txt, 
        ...
        self.caption_criterion_reduction = nn.CrossEntropyLoss(reduce=False)
        self.caption_criterion = nn.CrossEntropyLoss(ignore_index=0)
        ...
        ##initiate caption module
        captionModule = Caption(image_dim=in_image_dim, text_dim=embed_dim_txt, map_dim=hidden_size, vocab_size=num_c_vocab_txt)
        self.myCaptionNet = captionModule.cuda() if use_cuda else captionModule


    def forward(self, input_txt_variable, input_text_seq_lens,
                input_images, input_answers,
                input_layout_variable,sample_token, input_cap_variable, input_cap_seq_lens, 
                policy_gradient_baseline=None, baseline_decay=None):
        ...
        ##run caption module
        images_tensor = torch.FloatTensor(input_images)
        if use_cuda:
            images_tensor = images_tensor.cuda()
        images_variable = Variable(images_tensor)

        mask = torch.zeros(input_cap_variable.size(), dtype=torch.float32)
        for i in range(input_cap_variable.size(1)):
            mask[:input_cap_seq_lens[i], i] = 1
        ...
        predicted_logits = self.myCaptionNet(images_variable, input_cap_variable[:-1], input_cap_seq_lens)
        predicted_logits = predicted_logits.view(-1, predicted_logits.size(2))
        targets = input_cap_variable[1:].view(-1)

        #caption_loss = self.caption_criterion(predicted_logits, targets)
        caption_loss = self.caption_criterion_reduction(predicted_logits, targets)
        caption_loss = caption_loss.view(mask.size())

        caption_loss = caption_loss * mask
        caption_loss = torch.sum(caption_loss, 0)
        caption_loss = torch.mean(caption_loss, 0)
