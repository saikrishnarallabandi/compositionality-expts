...
class BatchLoaderVqa:
    def __init__(self, imdb, data_params):
        ...
        self.caption_dict = text_processing.VocabDict(data_params['vocab_captions_file'])
        self.C_encoder = data_params['C_encoder']
        ...
        fs = np.load(self.imdb[0]['feature_path'])
        for f in fs:
           feat = fs[f]  #   (2048, 14, 14)
        #print("The feats are: ", feat.shape) # 
        feat = feat.reshape(14,14,2048)  
        feats = np.zeros((1,14,14,2048), dtype='float32')
        feats[0] = feat
        #print("The feats are: ", feats.shape)      
    ...
    def load_one_batch(self, sample_ids):
        ...
        # captions
        input_c_seq_batch = np.zeros((self.C_encoder, actual_batch_size), np.int32)
        c_seq_length_batch = np.zeros(actual_batch_size, np.int32)
        ...
        for n in range(len(sample_ids)):
            ...
            caption_inds = [self.caption_dict.word2idx(w) for w in iminfo['caption_tokens']]
            caption_inds = [self.caption_dict.word2idx("<start>")] + caption_inds + [self.caption_dict.word2idx("<end>")]
            c_seq_length = len(caption_inds)

            if c_seq_length > self.C_encoder:
                caption_inds[self.C_encoder-1] = self.caption_dict.word2idx("<end>")
                c_seq_length = self.C_encoder

            input_seq_batch[:seq_length, n] = question_inds
            seq_length_batch[n] = seq_length

            input_c_seq_batch[:c_seq_length, n] = caption_inds[:c_seq_length]
            c_seq_length_batch[n] = c_seq_length

            #print("Loading features from ", iminfo['feature_path'])

            fs = np.load(iminfo['feature_path'])
            for f in fs:
              feat = fs[f]  #   (2048, 14, 14)
            #print("The feats are: ", feat.shape) # 
            feat = feat.reshape(14,14,2048)  
            ...
            image_feat_batch[n:n+1] = feats #np.load(iminfo['feature_path'])
        ...
        batch = dict(input_seq_batch=input_seq_batch,
                     ...
                     input_c_seq_batch=input_c_seq_batch,
                     c_seq_length_batch=c_seq_length_batch)
        if self.load_answer:
